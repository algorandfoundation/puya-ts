...
export class HelloWorld extends Contract {
  // @expect-error Type tests/expected-output/intersection-types.algo.ts::IntersectionType cannot be used for storage
  ...
  // @expect-error @runtimeOnly{account:Account,id:Uint<32>} cannot be used as an ABI return type
  ...
  // @expect-error @runtimeOnly{account:Account,id:Uint<32>} cannot be used as an ABI param type
  ...
  cannotBeLogged(): void {
    ...
    // @expect-error @runtimeOnly{account:Account,id:Uint<32>} cannot be encoded to an ARC4 type
    emit(account)
  }
