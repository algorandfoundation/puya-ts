/**
 * Example 12: Event Logger — Deployment & Test Script
 *
 ...
 * - Tests logTransfer — emit Transfer event with native uint64 fields (emit<T>)
 * - Tests logApproval — emit Approval typed named event (emit<T>)
 * - Tests logDeposit — emit Deposit event with positional native args
 * - Tests logWithdrawal — emit Withdrawal event with explicit ARC-28 signature
 * - Tests logStatusChange — emit StatusChanged event with string and uint64 fields
...
async function main() {
  printHeader('Example 12 — Event Logger')

  // Step 1: Compile the contract
  printStep(1, 'Compiling contract...')
  compileContract('12-event-logger')
  printSuccess('Contract compiled')

  // Step 2: Load app spec
  printStep(2, 'Loading app spec...')
  const appSpec = loadAppSpec('12-event-logger/out', 'EventLogger')
