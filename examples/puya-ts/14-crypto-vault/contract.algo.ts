...
import { assert, Contract, contract, ensureBudget, GlobalState, OpUpFeeSource, Uint64 } from '@algorandfoundation/algorand-typescript'
...
export class CryptoVault extends Contract {
   ...
   * @param commitment - the secret value to hash and store as the vault commitment
   */
   ...
   * @param data - the input data to hash with all four algorithms
   * @returns concatenation of all four hash outputs (128 bytes total)
   */
   ...
   * @param iterations - the uint64 value to store, load, and double via scratch space
   * @returns the doubled value read back from scratch space
   */
   ...
   * @param preimage - the candidate preimage to check against the stored hash
   * @returns true if the preimage hashes to the stored commitment
   */
