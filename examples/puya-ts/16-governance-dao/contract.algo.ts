...
export class GovernanceDao extends Contract {
  ...
  /** Deploy the contract; GlobalState values are auto-initialized. */
  ...
  /**
   * Configure DAO parameters (quorum and voting period); creator-only, one-time setup.
   * @param quorum - minimum yes-votes required to execute a proposal
   * @param votingPeriod - number of rounds a proposal stays open for voting
   */
  ...
  /**
   * Submit a new proposal and emit a ProposalCreated event; returns the proposal ID.
  ...
  /**
   * Execute a proposal that reached quorum after voting ends.
  ...
  /**
   * Count and return the number of proposals that met quorum.
   * @returns the number of proposals that reached the quorum threshold
   */
  public tally(): uint64 {
  ...
  /** Shut down the DAO, delete all proposal boxes, and emit a DaoClosed event; creator-only. */
  public close(): void {
