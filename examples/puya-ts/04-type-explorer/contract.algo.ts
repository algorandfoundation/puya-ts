...
export class TypeExplorer extends Contract {
   ...
   * @param a - first uint64 operand
   * @param b - second uint64 operand
   * @returns the sum after a uint64 → bytes → uint64 round trip
   */
   ...
   * @param value - uint64 to convert and square
  ...
  /**
   * Demonstrate Bytes() factory, Bytes.fromHex(), op.sha256, and bytes-to-biguint conversion.
   * @param input - arbitrary bytes to hash
   * @returns SHA-256 hash of the concatenated input
   */
   ...
   * @param a - first uint64 operand
   * @param b - second uint64 operand
   * @returns the low 64 bits of the 128-bit sum
   */
