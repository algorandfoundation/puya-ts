...
describe('Pure op codes', async () => {
  ...
  describe('jsonRef', async () => {
    test.each([
      ['{"key0": "hello","key1": "world"}', "key0"],
      ['{"key0": 1,"key1": {"key2":2,"key2":"10"}, "key2": "test"}', "key2"],
      ['{"key0": "\\ud801\\udc37"}', "key0"],
      ['{"key0": "\\ud800\\ud800n"}', "key0"]
    ])('should be able to extract string from json', async (a, b) => {
      const avmResult = (await getAvmResultRaw(appClient, 'verify_json_ref_string', asUint8Array(a), asUint8Array(b)))!
      const result = op.JsonRef.jsonString(a, b)
      expect(asUint8Array(result)).toEqual(avmResult)
    })

    test.each([
      ['{"key0": 0,"key1": 1}', "key1"],
      ['{"key0": 1,"key1": {"key2":2,"key2":"10"}, "key2": 42}', "key2"],
      ['{"key0": 1, "key1": ' + MAX_UINT64 + "}", "key1"],
    ])('should be able to extract uint64 from json', async (a, b) => {
      const avmResult = await getAvmResult<uint64>(appClient, 'verify_json_ref_uint64', asUint8Array(a), asUint8Array(b))
      const result = op.JsonRef.jsonUint64(a, b)
      expect(result.valueOf()).toEqual(avmResult)
    })

    test.each([
      ['{"key0": 1,"key1": {"key2":2,"key2":"10"}, "key2": "test"}', "key1"],
      ['{"key1": {"key2":2,"key2":"10"}}', "key1"],
      [
        '{"key0":1,"key1":{"key2":2,"key2":{"key5":"abc","key6":{"key7":"def","key7":"jkl"}}},"key2":"test"}',
        "key1",
      ],
      ['{"key0": {"key1":"\\ud801\\udc37"}}', "key0"],
      ['{"key0": {"key1":"\\uD800\\uD800n"}}', "key0"],
    ])('should be able to extract object from json', async (a, b) => {
      const avmResult = (await getAvmResultRaw(appClient, 'verify_json_ref_object', asUint8Array(a), asUint8Array(b)))!
      const result = op.JsonRef.jsonObject(a, b)
      expect(asUint8Array(result)).toEqual(avmResult)
    })

    test.each([
      [`{"key0": 0,"key1": ${MAX_UINT64 + 1n}}`, "key1"],
      [`{"key0": 0,"key1": ${MAX_UINT512}}`, "key1"],
    ])('should throw error when uint64 overflows', async (a, b) => {
      await expect(getAvmResult<uint64>(appClient, 'verify_json_ref_uint64', asUint8Array(a), asUint8Array(b))).rejects.toThrow(/cannot unmarshal .* into Go value of type uint64/)
      expect(() => op.JsonRef.jsonUint64(a, b)).toThrow('Uint64 over or underflow')
    })

    test.each([
      ['{"key0": "hello","key1": "world"}', ""],
      ['{"key0": 1,"key1": {"key2":2,"key2":"10"}, "key2": "test"}', "key3"],
    ])('should throw error when the key is not found', async (a, b) => {
      const keyError = /key .* not found in JSON text/
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_string', asUint8Array(a), asUint8Array(b))).rejects.toThrow(keyError)
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_uint64', asUint8Array(a), asUint8Array(b))).rejects.toThrow(keyError)
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_object', asUint8Array(a), asUint8Array(b))).rejects.toThrow(keyError)

      expect(() => op.JsonRef.jsonString(a, b)).toThrow(keyError)
      expect(() => op.JsonRef.jsonUint64(a, b)).toThrow(keyError)
      expect(() => op.JsonRef.jsonObject(a, b)).toThrow(keyError)
    })

    it('should throw error when wrong return type is used', async () => {
      const a = '{"key0": 1,"key1": {"key2":2,"key2":"10"}, "key2": "test"}'
      const returnTypeError = (t: string) => new RegExp(`cannot unmarshal .* into Go value of type ${t}`)
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_string', asUint8Array(a), asUint8Array("key1"))).rejects.toThrow(returnTypeError('string'))
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_uint64', asUint8Array(a), asUint8Array("key2"))).rejects.toThrow(returnTypeError('uint64'))
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_object', asUint8Array(a), asUint8Array("key0"))).rejects.toThrow(returnTypeError('map\\[string\\]json.RawMessage'))

      expect(() => op.JsonRef.jsonString(a, "key1")).toThrow('value must be a string type')
      expect(() => op.JsonRef.jsonUint64(a, "key2")).toThrow('value must be a numeric type')
      expect(() => op.JsonRef.jsonObject(a, "key0")).toThrow('value must be an object type')
    })
    ...
    test.each([
      ...
      ['{"key0": 0xFF}', "key0"],
      ['{"key": "algo",,,}', "key"],
      ['{"key0": [1,/*comment*/,3]}', "key0"],
      ['{"key0": 1,"key0": 2}', "key0"],
      ['{"key0": "\\uFF"}', "key0"],
      ['\\uFEFF{"key0": 1}', "key0"],
    ])('should throw error when input is invalid json', async (a, b) => {
      const jsonError = /error while parsing JSON text, invalid json text/
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_string', asUint8Array(a), asUint8Array(b))).rejects.toThrow(jsonError)
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_uint64', asUint8Array(a), asUint8Array(b))).rejects.toThrow(jsonError)
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_object', asUint8Array(a), asUint8Array(b))).rejects.toThrow(jsonError)

      expect(() => op.JsonRef.jsonString(a, b)).toThrow(jsonError)
      expect(() => op.JsonRef.jsonUint64(a, b)).toThrow(jsonError)
      expect(() => op.JsonRef.jsonObject(a, b)).toThrow(jsonError)
    })

    test.each([
      ['{"key": 1.2E-6}', "key"],
      ['{"key": 0.2E+8}', "key"],
      ['{"key0": 1,"key1": [2, "10"], "key2": "test"}', "key1"],
    ])('should throw error for unsupported types', async (a, b) => {
      const typeError = (t: string) => new RegExp(`cannot unmarshal .* into Go value of type ${t}`)
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_string', asUint8Array(a), asUint8Array(b))).rejects.toThrow(typeError("string"))
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_uint64', asUint8Array(a), asUint8Array(b))).rejects.toThrow(typeError("uint64"))
      await expect(getAvmResultRaw(appClient, 'verify_json_ref_object', asUint8Array(a), asUint8Array(b))).rejects.toThrow(typeError("map\\[string\\]json.RawMessage"))

      expect(() => op.JsonRef.jsonString(a, b)).toThrow('value must be a string type')
      expect(() => op.JsonRef.jsonUint64(a, b)).toThrow('value must be a numeric type')
      expect(() => op.JsonRef.jsonObject(a, b)).toThrow('value must be an object type')
    })
  })
})
