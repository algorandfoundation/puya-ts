...
function cli() {
  ...
  commander
    ...
    .addOption(new Option('--output-teal', 'Output TEAL code').default(true))
    .addOption(new Option('--output-arc32', 'Output {contract}.arc32.json ARC-32 app spec file').default(true))
    .addOption(new Option('--output-ssa-ir', 'Output IR (in SSA form) before optimisations').default(false))
    .addOption(new Option('--output-optimization-ir', 'Output IR after each optimization').default(false))
    .addOption(new Option('--output-destructured-ir', 'Output IR after SSA destructuring and before MIR').default(false))
    .addOption(new Option('--output-memory-ir', 'Output MIR before lowering to TealOps').default(false))
    .addOption(new Option('--output-bytecode', 'Output AVM bytecode').default(false))
    .addOption(new Option('--match-algod-bytecode', 'When outputting bytecode, ensure bytecode matches algod output').default(false))
    .addOption(
    ...
    .addOption(new Option('--template-vars-prefix [prefix]', 'Define the prefix to use with --template-var').default('TMPL_'))
    .addOption(
